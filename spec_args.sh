...
if [[ "${INCLUDE_API_FILING_PUBLIC_SPECS}" -eq "1" ]]; then
    ...
    SPEC_ARGS="${SPEC_ARGS} -spec-filename=api.ch.gov.uk-specifications/swagger-2.0/spec/testDataGenerator-public.json"
...
if [[ "${INCLUDE_PRIVATE_SPECS}" -eq "1" ]]; then
    ...
    SPEC_ARGS="${SPEC_ARGS} -spec-filename=api.ch.gov.uk-specifications/swagger-2.0/spec/testDataGenerator-private.json"
fi
